...
const WEIGHTS = {
  ...
  send:       40,   // POST /send                         — transfer (dominant write)
  createAcct: 30,   // POST /accounts                     — account creation
  deactivate: 10,   // PATCH /accounts/:id/deactivate     — low-frequency action
};
...
async function doSend(state) {
  ...
  await res.text();
  return { ok: res.ok, status: res.status, ms: now() - t0, endpoint: "POST /send" };
}
...
async function doDeactivate(state) {
  ...
  if (Math.random() < 0.25 && state.deactivated.length > 0) {
    ...
    return { ok, status: res.status, ms: now() - t0, endpoint: "PATCH /accounts/:id/deactivate" };
  }
